...
# EC2 cannot use an IAM role directly — instance profile is the required wrapper
...
############################
# 3. System Node Group Role
#    For the managed node group running Karpenter + CoreDNS
############################
resource "aws_iam_role" "system_node_group" {
...
resource "aws_iam_role_policy_attachment" "system_node_worker" {
  role       = aws_iam_role.system_node_group.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
}
